...
def FFD_heart(mesh_obj:Mesh, start_points, end_points):
    ...
    vol_0 = mesh_obj.get_volume()

    def ffd(params, sob_w):

        out_data = (mesh_obj.evaluate_ele_xi_pair_embeddings(elem, xis, fit_params=params) - end_points).flatten()
        sob_dif = (mesh_obj.evaluate_sobolev(fit_params=params) - pre_sob) * 0.1
        vol_dif = jnp.asarray([mesh_obj.get_volume(fit_params=params) - vol_0]) * sob_w

        return jnp.concatenate((out_data, sob_dif, vol_dif))
...
if __name__ == "__main__":
    ...
    np.save("bin/embeded_es.npy", embed_ele_xis[0])
    np.save("bin/embeded_xis.npy", embed_ele_xis[1])
    ...
    # f, j, init_params, embed_ele_xis = FFD_heart(mesh_obj, start_points, end_points)
    # res = fit_heart(f,j, init_params, 0.1, verbose=0)
    #
    # mesh_obj.plot()
    ...
    print(f"init vol {init_vol}")
    print(f"final vol {mesh_obj.get_volume()}")
    ...
    s.add_arrows(wpts, deltas, mag=5, cmap="turbo")
    s.show()

    xi_grid = mesh_obj.xi_grid(res=5, dim=3)
    # base_mesh.strain_tensor(mesh_obj, xi_grid)

    save_mesh(mesh_obj, 'bin/ffd_mesh_no_vol.json')
